...
fn do_counting(path: &std::path::Path, counter: &mut Counter){
...
fn walk_dir(path: &std::path::Path, counter: &mut Counter) {
    if path.is_dir() {
...
fn print_table(counter: &Counter) {
